...
export function getLessonsFor(year: YearLevel, subject: SubjectId, language?: LanguageId, csPathway?: import("@/lib/types").CsPathwayId): Lesson[] {
...
export function getLesson(year: YearLevel, subject: SubjectId, lessonId: string, language?: LanguageId): Lesson | undefined {
...
export function allLessonKeysForYear(year: YearLevel): string[] {
  return LESSONS.filter((l) => l.year === year).map((l) => l.language ? `${l.year}:${l.subject}:${l.language}:${l.id}` : `${l.year}:${l.subject}:${l.id}`);
}
